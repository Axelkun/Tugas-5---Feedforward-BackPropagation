...
def MLFF(dataset,wh,bh,wo,bo):
    ...
    for j in range(0, 100):
        ...
        for i in range(0, len(dataTrain)):
            output = FF(i,dataTrain,wh,bh,wo,bo)
        ...
        for i in range(0, len(dataValid)):
            outputV = FF(i,dataValid, wh, bh, wo, bo)
...
def MLFFSoftmax(dataset, wh, bh, wo, bo):    
    AvgErrTrain = []
    AvgErrValid = []
    for j in range(0, 100):
        dataTrain = []
        ...
        AvgErr = []
        for i in range(0, len(dataTrain)):
            output = FFSoftmax(i,dataTrain,wh,bh,wo,bo)
            ErrSeto.append(output[1][0])
            ErrVers.append(output[1][1])
            ErrVirg.append(output[1][2])
            wo = output[2]
        ...
        AvgErrV = []
        for i in range(0, len(dataValid)):
            outputV = FFSoftmax(i, dataValid,wh,bh,wo,bo)
            ErrSetoV.append(outputV[1][0])
            ErrVersV.append(outputV[1][1])
            ErrVirgV.append(outputV[1][2])
            AvgErrV.append((ErrSetoV[i] + ErrVersV[i] + ErrVirgV[i])/3)
...
MLFFSoftmax(dataset,wh,bh,wo,bo)
